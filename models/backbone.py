...
from models.position_encoding import build_position_encoding, build_position_encoding_original
...
#         print('ResNet pretrain: ImageNet')
        ...
        if name == 'resnet50':
            if load_backbone == 'supervised':
                backbone = getattr(torchvision.models, name)(
                    replace_stride_with_dilation=[False, False, dilation],
                    pretrained=False, norm_layer=FrozenBatchNorm2d)
                #default_sup_pretrain = torch.load('/PATH/TO/params/resnet50-19c8e357.pth', map_location='cpu')
            ...
            # load the SwAV pre-training model from the url instead of supervised pre-training model
                ...
                checkpoint = torch.load('/PATH/TO/params/swav_800ep_pretrain.pth.tar', map_location="cpu")
            ...
            if load_backbone == 'supervised':
                backbone = getattr(torchvision.models, name)(
                    replace_stride_with_dilation=[False, False, dilation],
                    pretrained=False, norm_layer=FrozenBatchNorm2d)
                default_sup_pretrain = torch.load('/PATH/TO/params/resnet101-5d3b4d8f.pth', map_location='cpu')
                backbone.load_state_dict(default_sup_pretrain, strict = True)
                print('Loading ResNet supervisedly pretrained on ImageNet from localdir...')
        else:
            backbone = getattr(torchvision.models, name)(
...
def build_backbone(args):
    ...
    backbone = Backbone(args.RLIP.backbone, train_backbone, return_interm_layers, args.RLIP.dilation, load_backbone=args.RLIP.load_backbone)
    model = Joiner(backbone, position_embedding)
    model.num_channels = backbone.num_channels
    return model

def build_backbone_original(args):
    position_embedding = build_position_encoding_original(args)
    train_backbone = args.lr_backbone > 0
    return_interm_layers = args.masks # or (args.num_feature_levels > 1)
