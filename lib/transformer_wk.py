...
class transformer_wk(nn.Module):
    ...
    def forward(self, features, im_idx):
        # weakly supervised的情况，注意这种情况下有些帧没有检测到标注
        # 这里采取的策略是不对这些没有标注的帧计算
        ...
        local_output, local_attention_weights = self.local_attention(rel_input, masks.type(torch.int))
        ...
        for j in range(b - 1):
            ...
            # idx global_input存储的feature所相关联的帧数(idx[pos]的值只有-1,pos和pos+1)
        ...
        if global_input.shape[1] == 0:
            return local_output, None, None
        # temporal decoder
        global_output_1, global_attention_weights = self.global_attention(global_input, global_masks, position_embed)
        output = torch.zeros_like(features)

        # 恢复global_output的大小
        global_output = torch.zeros([l * 2, b - 1, features.shape[1]]).to(features.device)   
...
class transformer_ours(nn.Module):
    ...
    def __init__(self, enc_layer_num=1, dec_layer_num=3, embed_dim=1936, nhead=8, dim_feedforward=2048,
                 dropout=0.1, mode=None):
        super(transformer_ours, self).__init__()
        self.mode = mode
    ...
    def forward(self, features, im_idx):
        ...
        # spatial encoder 
        local_output, local_attention_weights = self.local_attention(rel_input, masks.type(torch.bool))
        # local_output rel_num*feature_size
        local_output = (local_output.permute(1, 0, 2)).contiguous().view(-1, features.shape[1])[masks.view(-1) == 0]
        ...
        # print(global_input.shape, global_masks.shape, position_embed.shape)
        if global_input.shape[1] == 0:
            return local_output, None, None
        # temporal decoder
        global_output_1, global_attention_weights = self.global_attention(global_input, global_masks, position_embed)
