...
class VSGMeanRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, num_rel, ind_to_predicates, print_detail=False):
        super(VSGMeanRecall, self).__init__(result_dict)
    ...
    def register_container(self, mode):
        self.result_dict[mode + "_mean_recall"] = {10: 0.0, 20: 0.0, 50: 0.0}
        ...
        self.result_dict[mode + "_mean_recall_list"] = {10: [], 20: [], 50: []}
    ...
    def generate_print_string(self, mode):
        ...
        if self.print_detail:
            result_str += "Per-class recall@10: \n"
            for n, r in zip(
                self.rel_name_list, self.result_dict[mode + "_mean_recall_list"][10]
            ):
    ...
    def collect_mean_recall_items(self, pred_to_gt, gt_entry, pred_entry, mode):
    ...
    def calculate_mean_recall(self, mode):
        for k, v in self.result_dict[mode + "_mean_recall"].items():
            ...
            self.result_dict[mode + "_mean_recall"][k] = sum_recall / float(num_rel)


class VSGNGMeanRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, num_rel, ind_to_predicates, print_detail=False):
        super(VSGNGMeanRecall, self).__init__(result_dict)
        self.num_rel = num_rel
        self.print_detail = print_detail
    ...
    def register_container(self, mode):
        self.result_dict[mode + "_ng_mean_recall"] = {10: 0.0, 20: 0.0, 50: 0.0}
        self.result_dict[mode + "_ng_mean_recall_collect"] = {
            10: [[] for i in range(self.num_rel)],
            20: [[] for i in range(self.num_rel)],
            50: [[] for i in range(self.num_rel)]
        }
        self.result_dict[mode + "_ng_mean_recall_list"] = {10: [],20: [], 50: []}
    ...
    def generate_print_string(self, mode):
        ...
        if self.print_detail:
            result_str += "----------------------- Details ------------------------\n"
            for n, r in zip(
                self.rel_name_list, self.result_dict[mode + "_ng_mean_recall_list"][50]
            ):
                result_str += "({}:{:.4f}) ".format(str(n), r)
            result_str += "\n"
            result_str += "--------------------------------------------------------\n"
    ...
    def calculate_mean_recall(self, mode):
        for k, v in self.result_dict[mode + "_ng_mean_recall"].items():
            ...
            num_rel = self.num_rel
            for idx in range(num_rel):
                if len(self.result_dict[mode + "_ng_mean_recall_collect"][k][idx]) == 0:
                    tmp_recall = 0.0
                else:
                    tmp_recall = np.mean(
                        self.result_dict[mode + "_ng_mean_recall_collect"][k][idx]
                    )
                self.result_dict[mode + "_ng_mean_recall_list"][k].append(tmp_recall)
                sum_recall += tmp_recall

            self.result_dict[mode + "_ng_mean_recall"][k] = sum_recall / float(num_rel)
        return


class VSGRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, AG_attention_predicates, AG_spatial_predicates, AG_contacting_predicates):
        super(VSGRecall, self).__init__(result_dict)
        self.type = "recall"
        self.attention_range = np.arange(0, len(AG_attention_predicates))
        self.spatial_range = np.arange(len(AG_attention_predicates), len(AG_attention_predicates)+len(AG_spatial_predicates))
        self.contacting_range = np.arange(9, 9+len(AG_contacting_predicates))

    def register_container(self, mode):
        self.result_dict[mode + f"_{self.type}"] = {10: [], 20: [], 50: []}
    ...
    def calculate_recall(self, gt_entry,  pred_entry, mode):
        gt_rels = gt_entry['gt_relations']
        ...
        pred_to_gt, pred_tuples, rel_scores = evaluate_recall(
                    gt_rels, gt_boxes, gt_classes,
                    pred_rels, pred_boxes, pred_classes,
                    predicate_scores, obj_scores, phrdet= mode=='phrdet',
                    iou_thresh=0.5)
        for k in self.result_dict[mode + f"_{self.type}"]:

            match = reduce(np.union1d, pred_to_gt[:k])

            rec_i = float(len(match)) / float(gt_rels.shape[0])
            self.result_dict[mode + f"_{self.type}"][k].append(rec_i)
        return pred_to_gt




class VSGSemiRecall(SceneGraphEvaluation):
    def __init__(self, result_dict):
        super(VSGSemiRecall, self).__init__(result_dict)
        self.type = "semi_recall"
    ...
    def generate_print_string(self, mode):
        result_str = "SGG eval: "
        for k, v in self.result_dict[mode + "_semi_recall"].items():
            result_str += "  R @ %d: %.4f; " % (k, np.mean(v))
        result_str += " for mode=%s, type=Semi Recall." % mode
        result_str += "\n"
        return result_str
    ...
    def calculate_recall(self, gt_entry, pred_entry, mode):
        ...
        for i, j in enumerate(pred_rel_inds):
            if rel_scores[i,0]+rel_scores[i,1] > 0:
                ...
                for k in np.where(rel_scores[i]>0.5)[0]:
...
class VSGNoGraphConstraintRecall(SceneGraphEvaluation):
    def __init__(self, result_dict):
        super(VSGNoGraphConstraintRecall, self).__init__(result_dict)
...
class SceneGraphEvaluator:
    def __init__(self, mode, AG_object_classes, AG_all_predicates, AG_attention_predicates, AG_spatial_predicates, AG_contacting_predicates,
        ...
        self.result_dict = {}
        self.subject_category = 1
        self.eval_recall = VSGRecall(self.result_dict, AG_attention_predicates, AG_spatial_predicates, AG_contacting_predicates)
        self.eval_nog_recall = VSGNoGraphConstraintRecall(self.result_dict)
        self.eval_semi_recall = VSGSemiRecall(self.result_dict)
        self.eval_mean_recall = VSGMeanRecall(self.result_dict, len(AG_all_predicates), AG_all_predicates, print_detail=True)
        self.eval_nog_mean_recall = VSGNGMeanRecall(self.result_dict, len(AG_all_predicates), AG_all_predicates, print_detail=True)
        self.mode = mode
    ...
    def register_container(self):
        self.eval_recall.register_container(self.mode)
        self.eval_mean_recall.register_container(self.mode)
        self.eval_nog_recall.register_container(self.mode)
        self.eval_semi_recall.register_container(self.mode)
        self.eval_nog_mean_recall.register_container(self.mode)
        
        
    def print_stats(self, logger):
        logger.info('======================' + self.mode + '============================')
        result_str = ''
        result_str += self.eval_recall.generate_print_string(self.mode)
        result_str += self.eval_nog_recall.generate_print_string(self.mode)
        result_str += self.eval_semi_recall.generate_print_string(self.mode)
        result_str += self.eval_mean_recall.generate_print_string(self.mode)
        result_str += self.eval_nog_mean_recall.generate_print_string(self.mode)
        logger.info(result_str)
        
    def calculate_mean_recall(self):
        self.eval_mean_recall.calculate_mean_recall(self.mode)
        self.eval_nog_mean_recall.calculate_mean_recall(self.mode)
    ...
    def evaluate_scene_graph(self, gt, pred):
        ...
        for idx, frame_gt in enumerate(gt):
            ...
            gt_relations = []
            human_idx = 0
            gt_classes[human_idx] = self.subject_category
            ...
            pred_to_gt = self.eval_recall.calculate_recall(gt_entry, pred_entry, mode = self.mode)
            pred_to_gt_nog = self.eval_nog_recall.calculate_recall(gt_entry, pred_entry, mode = self.mode)
            self.eval_semi_recall.calculate_recall(gt_entry, pred_entry, mode = self.mode)
            self.eval_mean_recall.collect_mean_recall_items(pred_to_gt, gt_entry, pred_entry, mode=self.mode)
            self.eval_nog_mean_recall.collect_mean_recall_items(pred_to_gt_nog, gt_entry, pred_entry, mode=self.mode)
...
class BasicSceneGraphEvaluator:
    ...
    def reset_result(self):
        self.result_dict[self.mode + '_recall'] = {10: [], 20: [], 50: [], 100: []}
        self.result_dict[self.mode + '_mean_recall'] = {10: [], 20: [], 50: [], 100: []}
        
    def print_stats(self, logger):
        logger.info('======================' + self.mode + '============================')
        for k, v in self.result_dict[self.mode + '_recall'].items():
            logger.info('R@%i: %f' % (k, np.mean(v)))
...
def _compute_pred_matches(gt_triplets, pred_triplets,
    ...
    for gt_ind, gt_box, keep_inds in zip(np.where(gt_has_match)[0],
        ...
        if phrdet:
            ...
            gt_box = np.array(gt_box, dtype=np.float32); boxes = np.array(boxes, dtype=np.float32)
